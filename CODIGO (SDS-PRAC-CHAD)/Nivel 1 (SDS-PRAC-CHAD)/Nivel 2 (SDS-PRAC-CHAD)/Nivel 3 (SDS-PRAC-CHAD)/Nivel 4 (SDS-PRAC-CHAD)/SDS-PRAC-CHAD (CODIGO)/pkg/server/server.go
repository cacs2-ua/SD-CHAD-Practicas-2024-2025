...
// They are generated by hashing the plain text bucket names.
...
// The key for both buckets is the SHA3-256 hash of the username.
func (s *serverImpl) registerUser(req api.Request) api.Response {
	...
	// Store encrypted UUID in auth_uuid bucket.
	...
	// Store hashed password in auth_password bucket.
...
// loginUser verifies the hashed password in auth_password, retrieves the encrypted UUID in auth_uuid,
...
func (s *serverImpl) loginUser(req api.Request) api.Response {
	...
	// Store in refresh bucket with key = HashBytes(userUUID).
...
// refreshToken retrieves the stored hashed refresh token, compares it to the hash of the provided token,
// and, if valid, generates new tokens and stores the hashed new refresh token.
func (s *serverImpl) refreshToken(req api.Request) api.Response {
	if req.Username == "" || req.RefreshToken == "" {
		return api.Response{Success: false, Message: "Missing credentials"}
	}
	// Retrieve decrypted UUID using username.
	...
	// Compute the hash of the provided refresh token.
